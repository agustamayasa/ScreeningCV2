...
def ensure_folder_structure(drive, job_position: str):
    """
    Membuat struktur folder di Google Drive jika belum ada
    Returns: (main_folder_id, cv_folder_id)
    """
    try:
        # 1. Cari atau buat folder utama "AI Resume Screening"
        ...
        if files:
            ...
            print(f"Folder utama '{main_folder_name}' ditemukan")
        else:
            ...
            print(f"Folder utama '{main_folder_name}' berhasil dibuat")
        
        # 2. Cari atau buat folder screening untuk posisi tertentu
        screening_folder_name = f"Screening - {job_position}"
        screening_folder_id = None
        
        # Cari folder screening di dalam folder utama
        results = drive.files().list(
            q=f"name='{screening_folder_name}' and '{main_folder_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false",
            fields="files(id, name)"
        ).execute()
        
        files = results.get('files', [])
        if files:
            screening_folder_id = files[0]['id']
            print(f"Folder screening '{screening_folder_name}' ditemukan")
        else:
            # Buat folder screening jika belum ada
            file_metadata = {
                'name': screening_folder_name,
                'mimeType': 'application/vnd.google-apps.folder',
                'parents': [main_folder_id]
            }
            folder = drive.files().create(body=file_metadata, fields='id').execute()
            screening_folder_id = folder.get('id')
            print(f"Folder screening '{screening_folder_name}' berhasil dibuat")
        
        # 3. Cari atau buat folder CV di dalam folder screening
        cv_folder_name = "CV"
        cv_folder_id = None
        
        # Cari folder CV di dalam folder screening
        results = drive.files().list(
            q=f"name='{cv_folder_name}' and '{screening_folder_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false",
            fields="files(id, name)"
        ).execute()
        
        files = results.get('files', [])
        if files:
            cv_folder_id = files[0]['id']
            print(f"Folder CV ditemukan")
        else:
            # Buat folder CV jika belum ada
            file_metadata = {
                'name': cv_folder_name,
                'mimeType': 'application/vnd.google-apps.folder',
                'parents': [screening_folder_id]
            }
            folder = drive.files().create(body=file_metadata, fields='id').execute()
            cv_folder_id = folder.get('id')
            print(f"Folder CV berhasil dibuat")
        
        return screening_folder_id, cv_folder_id
        
    except Exception as e:
        print(f"Error creating folder structure: {e}")
        # Return None jika gagal, file akan disimpan di root
        return None, None
    

def upload_to_drive(drive, file_data, filename, cv_folder_id=None):
    """Upload file ke Google Drive dalam folder CV yang sudah ditentukan"""
    try:
        # Buat file metadata dengan parent folder jika ada
        file_metadata = {
            'name': f"CV_{filename}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        }
        
        # Tambahkan parent folder jika ada
        if cv_folder_id:
...
# ==============================================================================
# MODIFIKASI FUNGSI ensure_spreadsheet_exists
# ==============================================================================
...
def ensure_spreadsheet_exists(gc, drive, spreadsheet_name: str, job_position: str):
    ...
    try:
        # Dapatkan folder structure terlebih dahulu
        screening_folder_id, _ = ensure_folder_structure(drive, job_position)
        ...
        try:
            ...
            # Pindahkan ke folder yang sesuai jika belum ada di sana
            if screening_folder_id:
                try:
                    # Cek apakah spreadsheet sudah di folder yang benar
                    file_info = drive.files().get(fileId=spreadsheet.id, fields='parents').execute()
                    current_parents = file_info.get('parents', [])
                    
                    if screening_folder_id not in current_parents:
                        # Pindahkan ke folder yang benar
                        drive.files().update(
                            fileId=spreadsheet.id,
                            addParents=screening_folder_id,
            ...
            # Pindahkan spreadsheet ke folder yang sesuai
            if screening_folder_id:
                try:
                    drive.files().update(
                        fileId=spreadsheet.id,
                    ...
                    print(f"Spreadsheet berhasil dibuat dalam folder screening")
                except Exception as e:
                    print(f"Error moving new spreadsheet to folder: {e}")
            
            print(f"Spreadsheet '{spreadsheet_name}' berhasil dibuat!")
            return spreadsheet
            
    except Exception as e:
        print(f"Error in ensure_spreadsheet_exists: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to create/access spreadsheet: {str(e)}")
...
def update_progress(current, total, message=""):
    ...
    if not hasattr(app.state, 'screening_progress'):
        app.state.screening_progress = {}
...
async def screening_progress(request: Request):
    """Stream progress updates to frontend"""
    async def generate():
        try:
            while True:
                if hasattr(app.state, 'screening_progress'):
                    progress_data = app.state.screening_progress
...
async def start_screening(request: Request):
    ...
    try:
        ...
        update_progress(0, 100, "Mengambil data CV yang sudah diproses...")
        
        # Dapatkan semua data existing untuk pengecekan duplikasi yang lebih baik
        existing_data = sheet.get_all_records()
        existing_hashes = set()
        existing_emails = set()
        
        for row in existing_data:
            if 'CV_Hash' in row and row['CV_Hash']:
                existing_hashes.add(row['CV_Hash'])
            # Tambahkan pengecekan berdasarkan email juga
            if 'Email' in row and row['Email'] and row['Email'] != 'Tidak tercantum':
                existing_emails.add(row['Email'].lower().strip())
        
        # Build query berdasarkan subjek email
        gmail_query = build_gmail_query(email_subjects)
        print(f"Gmail query: {gmail_query}")
        
        update_progress(0, 100, "Mencari email dengan CV...")
        
        # Query Gmail untuk email dengan resume
        results = gmail.users().messages().list(
            userId='me', 
            q=gmail_query,
            maxResults=500  # Tingkatkan limit untuk mendapatkan lebih banyak email
        ).execute()
        
        messages = results.get('messages', [])
        
        # Paginate untuk mendapatkan semua email jika lebih dari 500
        ...
        if not messages:
            update_progress(0, 0, "Tidak ada email ditemukan")
            return JSONResponse(content={
        ...
        # Proses semua email yang ditemukan (atau batasi jika terlalu banyak)
        max_emails = min(100, len(messages))  # Tingkatkan dari 30 ke 100
        
        update_progress(0, max_emails, f"Memproses {max_emails} email...")
        
        for i, message in enumerate(messages[:max_emails]):
            try:
                current_progress = i + 1
                update_progress(current_progress, max_emails, f"Memproses email {current_progress}/{max_emails}...")
                
                # Print progress untuk monitoring
                if current_progress % 5 == 0:
                    print(f"Progress: {current_progress}/{max_emails} emails processed")
                ...
                # Jika tidak ada parts, coba cek payload langsung
                if not parts and 'body' in payload and 'attachmentId' in payload.get('body', {}):
                    parts = [payload]
                
                if not parts:
                    continue
                
                # Proses semua attachment PDF dalam email ini
                for part in parts:
                    filename = part.get('filename', '')
                    
                    # Cek apakah ini adalah PDF
                    ...
                    try:
                        ...
                        update_progress(current_progress, max_emails, f"Mengunduh CV: {filename}...")
                        ...
                        update_progress(current_progress, max_emails, f"Mengekstrak teks dari: {filename}...")
                        
                        resume_text = extract_text_from_pdf_bytes(file_data)
                        
                        if not resume_text or len(resume_text.strip()) < 100:
                            print(f"Gagal ekstrak teks dari {filename} atau teks terlalu pendek")
                            error_count += 1
                            continue
                        ...
                        # Periksa apakah CV sudah pernah diproses berdasarkan hash
                        if cv_hash in existing_hashes:
                            print(f"CV {filename} sudah pernah diproses (hash match), skip.")
                            skipped_count += 1
                            continue
                        
                        update_progress(current_progress, max_emails, f"Menganalisis: {filename}...")
                        ...
                        # Cek apakah email kandidat sudah ada (double check)
                        candidate_email = analysis_result.get('email', 'Tidak tercantum')
                        if candidate_email != 'Tidak tercantum' and candidate_email.lower().strip() in existing_emails:
                            print(f"Kandidat dengan email {candidate_email} sudah pernah diproses, skip.")
                            skipped_count += 1
                            continue
                        
                        update_progress(current_progress, max_emails, f"Mengupload ke Drive: {filename}...")
                        ...
                        update_progress(current_progress, max_emails, f"Menyimpan data: {filename}...")
                        
                        sheet.append_row(row_to_insert)
                        existing_hashes.add(cv_hash)
                        if candidate_email != 'Tidak tercantum':
                            existing_emails.add(candidate_email.lower().strip())
        ...
        update_progress(max_emails, max_emails, "Screening selesai!")
        
        message = f"✓ {processed_count} CV baru berhasil diproses, {skipped_count} CV sudah ada sebelumnya"
        if error_count > 0:
            message += f", {error_count} CV gagal diproses"
        message += f" dari {max_emails} email yang diperiksa (total {len(messages)} email ditemukan)."
        ...
        update_progress(0, 0, f"Error: {e.detail}")
        raise e
    except Exception as e:
